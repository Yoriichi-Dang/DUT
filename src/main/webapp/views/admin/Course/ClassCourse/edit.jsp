...
<div class="container-xxl flex-grow-1 container-p-y">
  ...
  <h4 class="fw-bold py-3 mb-4"><span  class="text-muted fw-light">${model.code} /</span> Cập nhật lớp học phần</h4>
  <div class="row">
    <div class="col-xl">
      <div class="card mb-4">
        ...
        <div class="card-body">
          <form id="formAccountSettings" method="POST" onsubmit="return false">
            <div class="row">
              <input class="form-control" type="hidden" id="idClassCourse" name="firstName" value="${model.id}" autofocus="">
              <input class="form-control" type="hidden" id="codeCourse" name="firstName" value="${model.courseCode}" autofocus="">
              <input class="form-control" type="hidden" id="codeClassCourse" name="firstName" value="${model.code}" autofocus="">
              ...
              <select id="startWeek" class="form-select" required="">
                <c:forEach var="item" begin="1" end="45">
                  <option value="${item}" ${model.startWeek!=null and model.startWeek eq item ? "selected":""}>${item}</option>
                </c:forEach>
              </select>
            </div><div class="mb-3 col-md-3">
              ...
              <select id="endWeek" class="form-select" required="">
                <c:forEach var="item" begin="1" end="45">
                  <option value="${item}" ${model.endWeek!=null and model.endWeek eq item ? "selected":""}>${item}</option>
                </c:forEach>
              </select>
            </div>
              <div class="mb-3 col-md-3">
                ...
                <select id="dayOnWeek" class="form-select" required="">
                  <c:forEach var="item" begin="1" end="7">
                    ...
                    <option value="${item}" ${model.dayOnWeek!=null and model.dayOnWeek eq item?"selected":""}>
                      <c:if test="${item eq 1}">
                        Chủ nhật
                      </c:if>
                      <c:if test="${item != 1}">
              ...
              <div class="mb-3 col-md-2">
                ...
                <select id="startLesson" class="form-select" required="">
                  <c:forEach var="item" begin="1" end="12">
                    <option value="${item}" ${model.startLesson!=null and model.startLesson eq item?"selected":""}>${item}</option>
                  </c:forEach>
                </select>
              </div> <div class="mb-3 col-md-2">
              <label for="endLesson" class="form-label">Tiết kết thúc</label>
              <select id="endLesson" class="form-select" required="">
                <c:forEach var="item" begin="1" end="12">
                  <option value="${item}" ${model.endLesson!=null and model.endLesson eq item?"selected":""}>${item}</option>
                </c:forEach>
              </select>
            </div>
              <div class="mb-3 col-md-2">
                <label for="slotRoom" class="form-label">Chỗ</label>
                <input value="${model.slotRoom}" name="id" type="number" id="slotRoom" class="form-control"  required min="1">
              </div>
              <div class="mb-3 col-md-6">
                ...
                <select id="teacherCode" class="form-select" required="">
                  <c:forEach var="item" items="${teachers}" varStatus="loop">
                    <option value="${item.code}" ${model.teacherCode!=null and model.teacherCode eq item?"selected":""}>
                      K.${item.facultyModel.name}  -  ${item.name}
                    </option>
                  </c:forEach>
                </select>
              </div> <div class="mb-3 col-md-6">
                <label for="roomCode" class="form-label">Phòng học</label>
                <select id="roomCode" class="form-select" required="">
                  <c:forEach var="item" items="${rooms}" varStatus="loop">
                    <option data-seat="${item.seat}" value="${item.areaCode}${item.roomCode}" ${model.roomModel.areaCode eq item.areaCode and model.roomModel.roomCode eq item.roomCode ? "selected":""}>
                      ${item.areaCode}${item.roomCode}
                    </option>
                  </c:forEach>
                </select>
              </div>
            ...
            <div class="mt-2">
              ...
              <button onclick="window.location.href='<c:url value='/admin-class-course-page?type=classcourse&courseCode=${model.courseCode}'/>'" type="reset" class="btn btn-outline-secondary">Back</button>
            </div>
          </form>
        </div>
      </div>
...
<script>
  $("#btnAdd").on("click",function (e){
    ...
    let selectedValue = $("#roomCode").val();
    let slot=parseInt($("#slotRoom").val());
// Lấy giá trị thuộc tính tùy chỉnh data-seat
    let seatValue = parseInt($("#roomCode option:selected").data("seat"));
    if (startWeek < endWeek&&startLesson<endLesson) {

      var data={
        id:$("#idClassCourse").val(),
        code:$("#codeClassCourse").val(),
        startWeek:$("#startWeek").val(),
        endWeek:$("#endWeek").val(),
        dayOnWeek:$("#dayOnWeek").val(),
      ...
      if (isEmpty) {
        ...
        if(slot<=seatValue){
          updateClassCourse(data);
        }else{
          $("#selectTypeOpt").val("bg-warning");
          $("#selectPlacement").val("top-0 start-50 translate-middle-x");
          $("#contentToast").text("Phòng "+selectedValue+" chỉ có "+seatValue+" chỗ");
          $("#showToastPlacement").click();
        }
      }
    }else{
  ...
  function updateClassCourse(data){
    $.ajax({
      url:'${APIurl}',
      type:'PUT',
      ...
      error:function(error){
        ...
        $("#selectPlacement").val("top-0 start-50 translate-middle-x");
        $("#contentToast").text("Cập nhật thất bại vì "+ errorMessage);
        $("#showToastPlacement").click();
      }
    })
  }
